...
const FormBuilder = () => {
  ...
  const [Jsondata, setData] = useState(state);
  const [jsonresult, setresult] = useState(state?.jsonData);
  ...
  const initializeFormBuilder = () => {
    ...
    $(fb.current).formBuilder({
      ...
      // formData:!intValue ? Jsondata :JsonField
      formData: state?.name == null ? jsonresult : JsonField,
    });
  };
  ...
  const handleSaveForm = async () => {
    ...
    try {
      ...
      localStorage.setItem("custom-formid", data?.id);
      SetTemForm(data);
